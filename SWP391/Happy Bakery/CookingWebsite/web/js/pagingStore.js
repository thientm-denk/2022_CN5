...
let storeArr = window.sessionStorage.getItem('storeArray');
let storePageArr = JSON.parse(storeArr);
console.log(storePageArr);
let productArr = [];

productArr = storePageArr;
...
function renderProduct(product) {
    ...
    const content = product.map((item, index) => {
        if (index >= start && index < end) {
            ...
            html += '<a href="getUserProfile?checkedUserId='+ item.id +'" class="store-name">'+ item.name +'</a>';
            html += '<p class="store-more">Phone: '+ item.phone +'</p>';
            html += '<p class="store-more">Address: '+ item.address +'</p>'
            html += '</div>';
            html += '</div>';
            return html;
        }
