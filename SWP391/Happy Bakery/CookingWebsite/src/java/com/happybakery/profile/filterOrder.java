...
public class filterOrder extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            int cvUserId = Integer.parseInt(userId);
            int cvStatus = Integer.parseInt(orderStatus);
            
            if (userType.equalsIgnoreCase("store")) {
                try {
                    orderList =  OrderDAO.getOrdersStoreByStatus(cvUserId, cvStatus);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            } else {
                try {
                    orderList =  OrderDAO.getOrdersUserByStatus(cvUserId, cvStatus);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            }
